...
- Every Focus Space card now has an on/off switch. Turning a space on first
  shows what it will block; turning it off asks you to type the stop-early
  text.
- Focus Spaces now run manually, daily or weekly. A manual space runs until
  you turn it off, and schedules repeat until you stop them or until a date
  you choose. This replaces the Timer.
- Turning off a scheduled Focus Space now keeps its schedule, ready to switch
  back on, instead of deleting it.
- Pause has been replaced by "Turn back on after": each Focus Space can switch
  itself back on 5 minutes to 24 hours after you stop it, or stay off. The
  default pause length in Settings has gone with it.
- Quick Start has been removed. Blocking now always starts from a saved Focus
  Space, and a Quick Start block that is running when you update carries on as
  a normal Focus Space.
- To stop early, you now type words on every platform, with an estimate of how
  long it takes. Random gibberish and Max difficulty have been removed.
- [desktop] A new "Maximum words to stop early" setting lets you require up to
  1,000 words.
- A Focus Space that is switched off can now be edited in full, including its
  Editing option (previously called Strictness).
- Adding a website with "www." now also blocks it without, so
  www.youtube.com blocks youtube.com too. This also applies to websites in
  imported Focus Spaces.
- New installs now start with no Focus Spaces, so you set up your own.
- [desktop] In Allow-only Focus Spaces, the apps you allow now stay open and
  other apps are closed, as intended. Apps get the chance to save your work
  before they close, and installers and disk tools are never closed.
- [desktop] Apps are now shown by the names you know them by, and apps such as
  WhatsApp are blocked and allowed reliably.
- [desktop] The "Let's go!" screen now names the Focus Space that just
  started, shows the apps an Allow-only Focus Space keeps open, and names the
  apps it will close correctly.
- [macos] Firefox is now found wherever it is installed, so blocking in
  Firefox can be set up even when it is not in the Applications folder.
- [windows] Each account on a shared computer now has its own Focus Spaces,
  schedules and settings. The first time you open the app after updating, it
  copies the rules already set up on this computer into your account,
  including start-alert pictures and voice clips.
- [ios] Turning off one Focus Space no longer lifts the blocks of other Focus
  Spaces that are still running.
- [ios] Allow-only Focus Spaces now check Apple's limit of 50 allowed websites
  and 50 allowed apps across all running spaces together, including when you
  turn one back on.
- [ios] Blocking keeps working when Screen Time's app choices expire. If an
  app choice cannot be recovered, you are asked to pick it again.
- [ios] If Screen Time permission is removed, the app now notices and asks for
  it again.

### Privacy

- Digital Habits: Blocker now sends one anonymous count a day, only on days
  you use it, so we know how many people use it. It contains nothing about
  your Focus Spaces, and you can turn it off in Settings under "Send
  anonymous usage count". The welcome screen explains this under "Private by
  design".

### Fixes & Polish

- The Focus Space cards have been redesigned. Each card shows its status and
  times, such as "Blocking now" or "Paused until 16:00 tomorrow", and the
  "Show what it blocks on the card" option now works as described. Duplicate
  and Delete are at the top of the edit screen (and on right-click on
  desktop), and undoing a delete brings back the schedule too.
- The design of the create / edit Focus Space screen has been improved. It is
  now one screen with sections for what to block, when to block and how to
  stop early.
- Dialogs now dim the screen behind them more clearly, especially in dark
  mode, and no longer close when a drag ends outside them.
- [desktop] The title bar has been simplified. What is blocking now shows on
  each Focus Space card, and on Mac, double-clicking the title bar no longer
  selects everything in the app.
- [desktop] The in-app notice about new versions, which had stopped
  appearing, works again.
- [desktop] Diagnostics now describes Allow-only Focus Spaces correctly.
- [windows] The window no longer drops out of maximised when it regains focus
  or Settings is opened.
- [ios] The design of the iPhone and iPad screens has been improved. Swiping
  from the left edge goes back, text is no longer selected by accident, Save
  stays pinned at the bottom of the edit screen, and the stop-early progress
  bar starts empty.
- Chinese translations have been improved.

### Internal

- Download links and the feedback link now use the project's new web address.
- The usage count is left out of development and test builds.
- Automated tests now also run on Windows.
- Automated iOS builds now use Xcode 26.6.
- Developer documentation now matches the current code.
- Added automated tests for Allow-only Focus Spaces, schedules and the editor.
- Updated dependencies, including Vite, Vitest and Android build tools.
...
- [windows] The app no longer runs a background task every few minutes to
  check it is still running. This uses fewer resources and stops Windows
  Defender wrongly reporting the app as a threat. The Microsoft Store
  version now starts at sign-in through Windows' own Startup settings.

### Fixes & Polish

- [desktop] On narrow windows, clicking a Focus Space card now opens it as
  expected.

### Internal

- Fixed Microsoft Store uploads failing during publishing.
- Continuous integration now installs the Chromium version that matches
  the browser tests.
